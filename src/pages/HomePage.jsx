...
const HomePage = () => {
  ...
  const { categories, getCategories, getOneCourses, rating } = useLesson();

  // Добавленное состояние для управления выбранным рейтингом
  ...
  const handleRatingChange = (e, value, title) => {
    if (e.type === "click") {
      e.preventDefault();
    }
    e.stopPropagation();
    ...
    let keyName = Object.keys(selectedRating)[0];
    let valueName = Object.values(selectedRating)[0];

    let email = JSON.parse(localStorage.getItem("email"));
    console.log(selectedRating.keyName);
    rating(keyName, email, valueName);
  };

  const handleRatingClick = (e, title) => {
    e.preventDefault();

    const selectedRating = selectedRating[title];

    // if (selectedRating !== undefined) {
    //   rating(title, selectedRating);

    //   console.log(`Selected rating for ${title}: ${selectedRating}`);
    // } else {
    //   console.log(`Rating not selected for ${title}`);
    // }
  };
  return (
    <>
              ...
              <div className="rate">
                {[5, 4, 3, 2, 1].map((i) => (
                  <React.Fragment key={i}>
                    <input
                      type="radio"
                      id={`star${i}-${elem.title}`}
                      name={`rate-${elem.title}`}
                      value={i}
                      onChange={(e) => handleRatingChange(e, i, elem.title)}
                      checked={selectedRating[elem.title] === i}
                    />
                    <label htmlFor={`star${i}-${elem.title}`} title="text">
                      {i} stars
                    </label>
                  </React.Fragment>
                ))}
                <button
                  ...
                  onClick={(e) => handleRatingClick(e, elem.title)}
                ></button>
              </div>
            </div>
          ))}
